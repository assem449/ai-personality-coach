'use client';

import { useEffect, useState } from 'react';
import {
  Chart as ChartJS,
  CategoryScale,
  LinearScale,
  PointElement,
  LineElement,
  Title,
  Tooltip,
  Legend,
  BarElement,
  ArcElement,
} from 'chart.js';
import { Line, Bar, Doughnut } from 'react-chartjs-2';

ChartJS.register(
  CategoryScale,
  LinearScale,
  PointElement,
  LineElement,
  BarElement,
  ArcElement,
  Title,
  Tooltip,
  Legend
);

interface SentimentData {
  date: string;
  sentiment: string;
  motivationLevel: number;
  entryCount: number;
}

interface HabitData {
  _id: string;
  title: string;
  category: string;
  tracking: Record<string, boolean>;
  completed: number;
  streak: number;
}

export default function DashboardChart() {
  const [chartType, setChartType] = useState<'mood' | 'habits'>('mood');
  const [sentimentData, setSentimentData] = useState<SentimentData[]>([]);
  const [habitData, setHabitData] = useState<HabitData[]>([]);
  const [isLoading, setIsLoading] = useState(true);
  const [error, setError] = useState('');

  useEffect(() => {
    if (chartType === 'mood') {
      fetchSentimentData();
    } else {
      fetchHabitData();
    }
  }, [chartType]);

  const fetchSentimentData = async () => {
    try {
      setIsLoading(true);
      const response = await fetch('/api/journal/sentiment');
      const data = await response.json();

      if (data.success) {
        setSentimentData(data.data);
      } else {
        setError(data.error || 'Failed to fetch sentiment data');
      }
    } catch (error) {
      console.error('Error fetching sentiment data:', error);
      setError('Failed to load sentiment data');
    } finally {
      setIsLoading(false);
    }
  };

  const fetchHabitData = async () => {
    try {
      setIsLoading(true);
      const response = await fetch('/api/habits');
      const data = await response.json();

      if (data.success) {
        setHabitData(data.habits);
      } else {
        setError(data.error || 'Failed to fetch habit data');
      }
    } catch (error) {
      console.error('Error fetching habit data:', error);
      setError('Failed to load habit data');
    } finally {
      setIsLoading(false);
    }
  };

  const getLast7Days = () => {
    const days = [];
    for (let i = 6; i >= 0; i--) {
      const date = new Date();
      date.setDate(date.getDate() - i);
      days.push(date.toISOString().split('T')[0]);
    }
    return days;
  };

  const formatDate = (dateString: string) => {
    const date = new Date(dateString);
    return date.toLocaleDateString('en-US', { 
      weekday: 'short', 
      month: 'short', 
      day: 'numeric' 
    });
  };

  if (isLoading) {
    return (
      <div className="flex items-center justify-center h-64">
        <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-indigo-600"></div>
      </div>
    );
  }

  if (error) {
    return (
      <div className="text-center text-slate-500 py-8">
        <p className="mb-3">{error}</p>
        <button 
          onClick={() => chartType === 'mood' ? fetchSentimentData() : fetchHabitData()}
          className="px-4 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition-colors"
        >
          Retry
        </button>
      </div>
    );
  }

  // Mood Chart Data
  const renderMoodChart = () => {
    if (sentimentData.length === 0) {
      return (
        <div className="text-center py-12">
          <p className="text-slate-600 mb-4">No journal entries found.</p>
          <a 
            href="/journal" 
            className="px-4 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition-colors"
          >
            Start Journaling
          </a>
        </div>
      );
    }

    const labels = sentimentData.map(item => {
      const date = new Date(item.date);
      return date.toLocaleDateString('en-US', { month: 'short', day: 'numeric' });
    });

    const motivationData = sentimentData.map(item => item.motivationLevel);
    const sentimentColors = sentimentData.map(item => {
      switch (item.sentiment) {
        case 'positive': return '#10B981'; // emerald
        case 'negative': return '#EF4444'; // red
        default: return '#6B7280'; // gray
      }
    });

    const chartData = {
      labels,
      datasets: [
        {
          label: 'Motivation Level (1-5)',
          data: motivationData,
          borderColor: '#6366F1',
          backgroundColor: 'rgba(99, 102, 241, 0.1)',
          borderWidth: 3,
          fill: true,
          tension: 0.4,
          pointBackgroundColor: sentimentColors,
          pointBorderColor: sentimentColors,
          pointRadius: 8,
          pointHoverRadius: 12,
          pointBorderWidth: 2,
        },
      ],
    };

    const options = {
      responsive: true,
      maintainAspectRatio: false,
      animation: {
        duration: 1000,
        easing: 'easeInOutQuart' as const,
      },
      plugins: {
        legend: {
          display: true,
          position: 'top' as const,
          labels: {
            usePointStyle: true,
            padding: 20,
            font: {
              size: 14,
              weight: 'bold' as const,
            },
          },
        },
        tooltip: {
          backgroundColor: 'rgba(255, 255, 255, 0.95)',
          titleColor: '#1F2937',
          bodyColor: '#374151',
          borderColor: '#E5E7EB',
          borderWidth: 1,
          cornerRadius: 8,
          displayColors: true,
          callbacks: {
            afterLabel: function(context: any) {
              const index = context.dataIndex;
              const sentiment = sentimentData[index]?.sentiment;
              return `Sentiment: ${sentiment}`;
            }
          }
        }
      },
      scales: {
        y: {
          beginAtZero: true,
          max: 5,
          ticks: {
            stepSize: 1,
            font: { size: 12 },
          },
          grid: {
            color: 'rgba(0, 0, 0, 0.05)',
          },
        },
        x: {
          ticks: {
            font: { size: 12 },
          },
          grid: {
            color: 'rgba(0, 0, 0, 0.05)',
          },
        },
      },
    };

    const totalEntries = sentimentData.reduce((sum, item) => sum + item.entryCount, 0);
    const avgMotivation = sentimentData.length > 0 
      ? (sentimentData.reduce((sum, item) => sum + item.motivationLevel, 0) / sentimentData.length).toFixed(1)
      : '0';

    return (
      <div className="space-y-6">
        {/* Summary Stats */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="bg-gradient-to-br from-indigo-500 to-indigo-600 text-white rounded-xl p-4 shadow-lg">
            <div className="text-2xl font-bold mb-1">{totalEntries}</div>
            <div className="text-indigo-100 text-sm font-medium">Total Entries</div>
          </div>
          <div className="bg-gradient-to-br from-emerald-500 to-emerald-600 text-white rounded-xl p-4 shadow-lg">
            <div className="text-2xl font-bold mb-1">{avgMotivation}</div>
            <div className="text-emerald-100 text-sm font-medium">Avg Motivation</div>
          </div>
          <div className="bg-gradient-to-br from-purple-500 to-purple-600 text-white rounded-xl p-4 shadow-lg">
            <div className="text-2xl font-bold mb-1">{sentimentData.filter(item => item.entryCount > 0).length}</div>
            <div className="text-purple-100 text-sm font-medium">Active Days</div>
          </div>
        </div>

        {/* Chart */}
        <div className="h-80">
          <Line data={chartData} options={options} />
        </div>
        
        <p className="text-sm text-slate-600 text-center">
          Point colors indicate daily sentiment: 🟢 Positive, 🔴 Negative, ⚪ Neutral
        </p>
      </div>
    );
  };

  // Habits Chart Data
  const renderHabitsChart = () => {
    if (habitData.length === 0) {
      return (
        <div className="text-center py-12">
          <p className="text-slate-600 mb-4">No habits found.</p>
          <a 
            href="/habits" 
            className="px-4 py-2 bg-teal-600 text-white rounded-lg hover:bg-teal-700 transition-colors"
          >
            Create Habits
          </a>
        </div>
      );
    }

    const days = getLast7Days();
    const labels = days.map(formatDate);

    // Calculate completion percentage for each habit over the last 7 days
    const habitCompletionData = habitData.map((habit, index) => {
      const completionRates = days.map(date => {
        const isCompleted = habit.tracking?.[date] || false;
        return isCompleted ? 100 : 0;
      });

      const colors = [
        '#3B82F6', // blue
        '#10B981', // emerald
        '#F59E0B', // amber
        '#EF4444', // red
        '#8B5CF6', // violet
        '#06B6D4', // cyan
      ];

      return {
        label: habit.title,
        data: completionRates,
        backgroundColor: colors[index % colors.length],
        borderColor: colors[index % colors.length],
        borderWidth: 2,
        borderRadius: 4,
        borderSkipped: false,
      };
    });

    const chartData = {
      labels,
      datasets: habitCompletionData,
    };

    const options = {
      responsive: true,
      maintainAspectRatio: false,
      animation: {
        duration: 1000,
        easing: 'easeInOutQuart' as const,
      },
      plugins: {
        legend: {
          display: true,
          position: 'top' as const,
          labels: {
            usePointStyle: true,
            padding: 20,
            font: {
              size: 14,
              weight: 'bold' as const,
            },
          },
        },
        tooltip: {
          backgroundColor: 'rgba(255, 255, 255, 0.95)',
          titleColor: '#1F2937',
          bodyColor: '#374151',
          borderColor: '#E5E7EB',
          borderWidth: 1,
          cornerRadius: 8,
          displayColors: true,
          callbacks: {
            label: function(context: any) {
              return `${context.dataset.label}: ${context.parsed.y}%`;
            }
          }
        }
      },
      scales: {
        y: {
          beginAtZero: true,
          max: 100,
          ticks: {
            stepSize: 25,
            font: { size: 12 },
            callback: function(value: any) {
              return value + '%';
            }
          },
          grid: {
            color: 'rgba(0, 0, 0, 0.05)',
          },
        },
        x: {
          ticks: {
            font: { size: 12 },
          },
          grid: {
            color: 'rgba(0, 0, 0, 0.05)',
          },
        },
      },
    };

    const totalCompleted = habitData.reduce((sum, habit) => sum + habit.completed, 0);
    const avgStreak = habitData.length > 0 
      ? (habitData.reduce((sum, habit) => sum + habit.streak, 0) / habitData.length).toFixed(1)
      : '0';

    return (
      <div className="space-y-6">
        {/* Summary Stats */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="bg-gradient-to-br from-teal-500 to-teal-600 text-white rounded-xl p-4 shadow-lg">
            <div className="text-2xl font-bold mb-1">{habitData.length}</div>
            <div className="text-teal-100 text-sm font-medium">Active Habits</div>
          </div>
          <div className="bg-gradient-to-br from-blue-500 to-blue-600 text-white rounded-xl p-4 shadow-lg">
            <div className="text-2xl font-bold mb-1">{totalCompleted}</div>
            <div className="text-blue-100 text-sm font-medium">Total Completions</div>
          </div>
          <div className="bg-gradient-to-br from-amber-500 to-amber-600 text-white rounded-xl p-4 shadow-lg">
            <div className="text-2xl font-bold mb-1">{avgStreak}</div>
            <div className="text-amber-100 text-sm font-medium">Avg Streak</div>
          </div>
        </div>

        {/* Chart */}
        <div className="h-80">
          <Bar data={chartData} options={options} />
        </div>
        
        <p className="text-sm text-slate-600 text-center">
          Shows completion percentage for each habit over the last 7 days
        </p>
      </div>
    );
  };

  return (
    <div className="bg-white rounded-xl p-6 border border-slate-200 shadow-sm">
      {/* Header with Toggle */}
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6">
        <div>
          <h3 className="text-2xl font-bold text-slate-900 flex items-center">
            <span className="text-3xl mr-3">📊</span>
            Your Progress Over Time
          </h3>
          <p className="text-slate-600 mt-1">
            Track your {chartType === 'mood' ? 'mood and motivation' : 'habit completion'} patterns
          </p>
        </div>
        
        {/* Toggle Buttons */}
        <div className="flex bg-slate-100 rounded-lg p-1 mt-4 sm:mt-0">
          <button
            onClick={() => setChartType('mood')}
            className={`px-4 py-2 rounded-md text-sm font-medium transition-all duration-200 ${
              chartType === 'mood'
                ? 'bg-white text-indigo-600 shadow-sm'
                : 'text-slate-600 hover:text-slate-900'
            }`}
          >
            😊 Mood
          </button>
          <button
            onClick={() => setChartType('habits')}
            className={`px-4 py-2 rounded-md text-sm font-medium transition-all duration-200 ${
              chartType === 'habits'
                ? 'bg-white text-teal-600 shadow-sm'
                : 'text-slate-600 hover:text-slate-900'
            }`}
          >
            ✅ Habits
          </button>
        </div>
      </div>

      {/* Chart Content */}
      <div className="min-h-[400px]">
        {chartType === 'mood' ? renderMoodChart() : renderHabitsChart()}
      </div>
    </div>
  );
} 